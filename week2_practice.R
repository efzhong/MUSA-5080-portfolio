library(tidyverse)
library(tidycensus)

# I'm going to load some data now

pa_income <- get_acs(
  geography = "county",
  variables = "B19013_001",
  state = "PA",
  year = 2023,
  survey = "acs5"
)
dim(pa_income)
glimpse(pa_income)
head(pa_income)

# I have 67 counties (rows).

pa_income$GEOID

as.numeric("01001")

# The leading zero was dropped.
filter(pa_income, estimate > 60000)

# I'm predicting less than 67.

filter(pa_income, moe > 3000)
filter(pa_income, estimate < 50000)

select(pa_income, NAME, estimate, moe)
# I don't think the # of rows will change.

select(pa_income, GEOID, estimate)

mutate(pa_income, moe_pct = moe / estimate * 100)

pa_income <- mutate(pa_income, moe_pct = moe / estimate * 100)
pa_income

# moe_pct is the percent of margin of error.

arrange(pa_income, moe_pct)
arrange(pa_income, desc(moe_pct))
# Cameron County

step1 <- filter(pa_income, moe_pct > 5)
step2 <- arrange(step1, desc(moe_pct))
step3 <- select(step2, NAME, estimate, moe, moe_pct)
step3

pa_income %>%
  filter(moe_pct > 5) %>%
  arrange(desc(moe_pct)) %>%
  select(NAME, estimate, moe, moe_pct)

worst <- pa_income %>%
  filter(moe_pct > 8) %>%
  arrange(estimate) %>%
  select(NAME, moe_pct)
worst

summary(pa_income$moe_pct)

pa_income <- mutate(pa_income, reliable = moe_pct < 5)

# two rows
pa_income %>%
  group_by(reliable) %>%
  summarize(n = n(),
            avg_income = mean(estimate))

pa_income <- pa_income %>%
  mutate(reliability = case_when(
    moe_pct < 3 ~ "High confidence",
    moe_pct < 6 ~ "Moderate",
    TRUE        ~ "Low confidence"
  ))
count(pa_income, reliability)
# High confidence is 26, low confidence is 7, and moderate is 34

pa_two <- get_acs(
  geography = "county",
  variables = c("B19013_001", "B01003_001"),
  state = "PA", year = 2023, survey = "acs5"
)

pa_two

# It is pulling 134 because it's now printing two rows for every county

pa_wide <- get_acs(
  geography = "county",
  variables = c(income = "B19013_001",
                pop    = "B01003_001"),
  state = "PA", year = 2023, survey = "acs5",
  output = "wide"
)

pa_wide
# E stands for estimate and M stands for margin of error

pa_wide %>%
  mutate(moe_pct = incomeM / incomeE * 100) %>%
  arrange(desc(moe_pct)) %>%
  select(NAME, popE, incomeE, moe_pct) %>%
  head(10)

# popE is population estimate. counties with lower popE tend to have higher moe_pct

# testing california now
ca_wide <- get_acs(
  geography = "county",
  variables = c(income = "B19013_001",
                pop    = "B01003_001"),
  state = "CA", year = 2023, survey = "acs5",
  output = "wide"
)
ca_wide

ca_wide %>%
  mutate(moe_pct = incomeM / incomeE * 100) %>%
  arrange(desc(moe_pct)) %>%
  select(NAME, popE, incomeE, moe_pct) %>%
  head(10)

pa_wide <- get_acs(
  geography = "tract",
  variables = c(income = "B19013_001",
                pop    = "B01003_001"),
  state = "PA", county = "Philadelphia", year = 2023, survey = "acs5",
  output = "wide"
)
pa_wide

pa_wide %>%
  mutate(moe_pct = incomeM / incomeE * 100) %>%
  arrange(desc(moe_pct)) %>%
  select(NAME, popE, incomeE, moe_pct) %>%
  head(10)
